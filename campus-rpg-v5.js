...
let sceneRef=null,apps=[],running=false,paused=false,followActive=true,currentApp=null,audioCtx=null;
...
class CampusScene extends Phaser.Scene{
 constructor(){
   super('CampusScene');
   this.students={};this.staffByStation={};this.staffHomes=new Map();this.tip=null;this.activeRing=null;this.ambient=[];this.dragging=false;this.dragStart=null;this.furn={};
 }
 preload(){
 ...
 drawRoom(id,s){
   ...
   s.tasks.forEach((task,i)=>{
     ...
     if(base==='Bob')spr.play(base+'-idle-anim');
 ...
 spawnApplicant(app,pos){
   ...
   const spr=this.add.sprite(0,0,app.base+'-idle',0).setScale(2.8).setTint(app.tint);spr.play(app.base+'-idle-anim');
 ...
 updateQueueCount(){const n=apps.filter(a=>a.status==='waiting').length;if(this.queueText)this.queueText.setText(n+' waiting');dom('waitingCount').textContent=n}
 showTip(x,y,text){
 ...
 setMovingSprite(sprite,moving,base,dirX=1){
   if(moving){sprite.setTexture(base+'-walk');sprite.play(base+'-walk-anim',true);sprite.setFlipX(dirX<0);}
   else{sprite.setFlipX(false);sprite.setTexture(base+'-idle');sprite.play(base+'-idle-anim',true);}
 }
 async walkContainer(c,points,follow=false){
   ...
   for(let i=1;i<points.length;i++){
     ...
     if(follow&&followActive)this.cameras.main.pan(p.x,p.y,380,'Sine.easeInOut');
     const d=Phaser.Math.Distance.Between(c.x,c.y,p.x,p.y);await new Promise(res=>this.tweens.add({targets:c,x:p.x,y:p.y,duration:Math.max(170,d*1.65),ease:'Linear',onComplete:res}));
   }
   this.setMovingSprite(spr,false,base,1);
 }
 standStaff(spr){
   spr.setTexture(spr.baseName+'-idle');spr.play(spr.baseName+'-idle-anim',true);spr.isSitting=false;
 }
 sitStaff(spr){
   if(spr.baseName!=='Bob'){spr.setTexture(spr.baseName+'-sit',0);spr.anims.stop();spr.isSitting=true;}
   else{spr.setTexture('Bob-idle');spr.play('Bob-idle-anim',true);}
   spr.setFlipX(false);
 }
 async walkStaff(spr,points){
   this.standStaff(spr);spr.setTexture(spr.baseName+'-walk');spr.play(spr.baseName+'-walk-anim',true);
   for(let i=1;i<points.length;i++){
     while(paused)await wait(70);
     const p=points[i];spr.setFlipX(p.x<spr.x);const d=Phaser.Math.Distance.Between(spr.x,spr.y,p.x,p.y);await new Promise(res=>this.tweens.add({targets:spr,x:p.x,y:p.y,duration:Math.max(160,d*1.55),ease:'Linear',onComplete:res}));
   }
   this.standStaff(spr);
 }
...
async function processNext(){
 ...
 running=true;currentApp=app;app.status='active';sceneRef.updateQueueCount();dom('processBtn').disabled=true;dom('pauseBtn').disabled=false;dom('activeCase').textContent=app.id;log('AI Orchestrator','APPLICANT_CALLED',app.name+' is called from the Admission queue.');sceneRef.focus(app);
 ...
 for(let i=1;i<route.length;i++){
   const from=route[i-1],to=route[i];updateEvent(app,STATIONS[from].label,'HANDOFF',STATIONS[from].tasks.slice(-1)[0],'Escort applicant','Walk through campus corridor',STATIONS[to].label);log(STATIONS[from].label+' Staff','HANDOFF',app.name+' is escorted to '+STATIONS[to].label+'.');await sceneRef.escort(app,from,to);await sceneRef.processStation(app,to);
 }
 app.status='completed';running=false;sceneRef.updateQueueCount();dom('processBtn').disabled=!nextWaiting();dom('pauseBtn').disabled=true;updateEvent(app,'Academic Handover','JOURNEY_COMPLETE','Management Reporting Agent','Complete lifecycle','Applicant handed to Academic','—');log('Management Reporting Agent','JOURNEY_COMPLETE',app.name+' completed the admission journey.');chime('complete');sceneRef.bubbleAt(sceneRef.students[app.id].x,sceneRef.students[app.id].y-55,'Journey complete ✓',1700);
}
function resetAll(){
 running=false;paused=false;currentApp=null;if(!sceneRef)return;sceneRef.resetApplicants();dom('processBtn').disabled=false;dom('pauseBtn').disabled=true;dom('pauseBtn').textContent='Pause';dom('activityFeed').innerHTML='<div class="empty">No activity yet.</div>';updateEvent(null,'Entrance','Waiting for applicant','—','—','—','—');sceneRef.cameras.main.setZoom(1.05).centerOn(720,770);
}
