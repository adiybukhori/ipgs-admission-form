...
const CHIBI_PROFILES=[
 {skin:0xf2c6a7,hair:0x30211e,hairStyle:'bob',glasses:false},
 {skin:0xd9a27f,hair:0x16191c,hairStyle:'short',glasses:true},
 {skin:0xf0b990,hair:0x5b3328,hairStyle:'wave',glasses:false},
 {skin:0xb97858,hair:0x211816,hairStyle:'crop',glasses:false},
 {skin:0xe8ad86,hair:0x70452e,hairStyle:'long',glasses:false},
 {skin:0x8f5b43,hair:0x161616,hairStyle:'curl',glasses:true},
 {skin:0xf1c2a0,hair:0x2c1d19,hairStyle:'bun',glasses:false},
 {skin:0xc88966,hair:0x3b241d,hairStyle:'side',glasses:false}
];
const STUDENT_UNIFORM=0x244f82;
const STUDENT_ACCENT=0xe2b858;
...
class CampusScene extends Phaser.Scene{
 ...
 createAnimations(){ /* V8 chibi actors use vector-part tweens rather than sprite-sheet animation. */ }
 ...
 drawCampusShell(){
   ...
   // V8 foreground campus details
   [310,720,1090,1610,2070,2470].forEach((x,i)=>{
     const pot=this.add.rectangle(x,205,30,24,0xb98554).setStrokeStyle(2,0x815b37).setDepth(3);
     const crown=this.add.circle(x,181,22,0x4f9a63,.96).setDepth(3);
     const crown2=this.add.circle(x+(i%2?11:-11),188,15,0x65ad72,.92).setDepth(3);
   });
   this.drawCorridor();
 }
 drawCorridor(){
 ...
 createChibiActor(x,y,opts={}){
   const p=CHIBI_PROFILES[opts.profileIndex%CHIBI_PROFILES.length]||CHIBI_PROFILES[0];
   const role=opts.role||'staff',uniform=opts.uniformColor||STUDENT_UNIFORM,accent=opts.accentColor||STUDENT_ACCENT;
   const actor=this.add.container(x,y).setDepth(opts.depth||18);
   const shadow=this.add.ellipse(0,31,38,11,0x223848,.16);

   const backpack=this.add.graphics();
   backpack.fillStyle(role==='student'?0x173a69:0x405d72,1).fillRoundedRect(-18,-4,36,34,9);
   backpack.lineStyle(2,role==='student'?0xd2a943:0x71899c,.85).strokeRoundedRect(-18,-4,36,34,9);
   backpack.setVisible(role==='student');

   const leftLeg=this.add.graphics(),rightLeg=this.add.graphics();
   [leftLeg,rightLeg].forEach((leg,idx)=>{
     leg.fillStyle(0x273847,1).fillRoundedRect(-6,-1,12,25,5);
     leg.fillStyle(0x1d242b,1).fillEllipse(0,23,15,7);
     leg.x=idx?9:-9;leg.y=14;
   });

   const body=this.add.graphics();
   body.fillStyle(0xffffff,1).fillRoundedRect(-19,-18,38,36,10);
   body.fillStyle(uniform,1).fillRoundedRect(-17,-11,34,29,8);
   body.fillStyle(0xffffff,.96).fillTriangle(-8,-13,0,-5,8,-13);
   body.fillStyle(accent,.95).fillRect(-2,-6,4,13);

   const leftArm=this.add.graphics(),rightArm=this.add.graphics();
   [leftArm,rightArm].forEach((arm,idx)=>{
     arm.fillStyle(uniform,1).fillRoundedRect(-5,-3,10,23,5);
     arm.fillStyle(p.skin,1).fillCircle(0,20,5);
     arm.x=idx?22:-22;arm.y=-8;arm.angle=idx?-7:7;
   });

   const head=this.add.circle(0,-38,23,p.skin,1).setStrokeStyle(2,0x9e6f57,.18);
   const earL=this.add.circle(-22,-37,5,p.skin,1),earR=this.add.circle(22,-37,5,p.skin,1);

   const hair=this.add.graphics();
   hair.fillStyle(p.hair,1).fillCircle(0,-48,23);
   if(p.hairStyle==='bob'||p.hairStyle==='long')hair.fillRoundedRect(-24,-49,48,26,11);
   if(p.hairStyle==='long'){hair.fillRoundedRect(-24,-41,12,37,6);hair.fillRoundedRect(12,-41,12,37,6);}
   if(p.hairStyle==='wave'){hair.fillCircle(-18,-35,8);hair.fillCircle(18,-35,8);hair.fillCircle(0,-57,11);}
   if(p.hairStyle==='bun'){hair.fillCircle(13,-65,10);hair.fillRoundedRect(-21,-52,42,19,9);}
   if(p.hairStyle==='curl'){[-17,-7,5,17].forEach(dx=>hair.fillCircle(dx,-56+(Math.abs(dx)%5),8));}
   if(p.hairStyle==='short'||p.hairStyle==='crop'||p.hairStyle==='side')hair.fillRoundedRect(-21,-57,42,20,10);
   if(p.hairStyle==='side')hair.fillTriangle(-19,-50,8,-60,18,-38);
   ...
   if(p.glasses){
     glasses.lineStyle(1.6,0x25323d,.9).strokeCircle(-8,-38,6).strokeCircle(8,-38,6).lineBetween(-2,-38,2,-38);
   }

   const lanyard=this.add.graphics();
   if(role==='staff'){
     lanyard.lineStyle(1.4,0x214c77,.8).lineBetween(-6,-13,0,4).lineBetween(6,-13,0,4);
     lanyard.fillStyle(0xffffff,1).fillRoundedRect(-6,2,12,9,2);
     lanyard.lineStyle(1,0x214c77,.8).strokeRoundedRect(-6,2,12,9,2);
   }

   const badge=this.add.circle(15,-8,4,accent,.95);badge.setVisible(role==='staff');
 ...
 createRoomStaff(id,s,r){
   ...
   s.tasks.forEach((task,i)=>{
     const p=coords[i]||[0,0],sx=s.x+p[0],sy=s.y+p[1];
     ...
     const actor=this.createChibiActor(sx,sy-24,{role:'staff',profileIndex:s.num+i,uniformColor:s.color,accentColor:0xe1b756,depth:18});
 ...
 spawnApplicant(app,pos){
   ...
   const profileIndex=app.queueIndex||APPS.findIndex(a=>a.id===app.id);
   const actor=this.createChibiActor(0,0,{role:'student',profileIndex,uniformColor:STUDENT_UNIFORM,accentColor:STUDENT_ACCENT,depth:2});
   const tag=this.add.text(31,-1,app.name,{fontFamily:'Arial',fontSize:'8px',fontStyle:'bold',color:'#173a69',backgroundColor:'#ffffffdd',padding:{x:5,y:3}}).setOrigin(0,.5).setAlpha(.42);
   c.add([actor,tag]);c.bodySprite=actor;c.app=app;c.setSize(110,86);
   c.setInteractive(new Phaser.Geom.Rectangle(-30,-55,130,92),Phaser.Geom.Rectangle.Contains,{useHandCursor:true});
   c.on('pointerover',()=>tag.setAlpha(1));c.on('pointerout',()=>tag.setAlpha(.42));
   c.on('pointerdown',()=>this.showTip(c.x+40,c.y-62,app.name+' · '+app.id+' · '+app.status.toUpperCase()));
   this.students[app.id]=c;
 }
 createAmbientStudents(){
   const configs=[
     {profile:5,route:[{x:1020,y:770},{x:1640,y:770},{x:1020,y:770}]},
     {profile:2,route:[{x:1940,y:850},{x:2190,y:850},{x:1940,y:850}]},
     {profile:6,route:[{x:520,y:855},{x:850,y:855},{x:520,y:855}]}
   ];
   configs.forEach((cfg,i)=>{
     const c=this.add.container(cfg.route[0].x,cfg.route[0].y).setDepth(25);
     const actor=this.createChibiActor(0,0,{role:'student',profileIndex:cfg.profile,uniformColor:0x466b91,accentColor:0xd6b05d,depth:1});
     actor.setScale(.82);c.add(actor);c.bodySprite=actor;this.ambient.push(c);this.setChibiMoving(actor,true,1);
     this.loopAmbient(c,cfg.route,i*650);
   });
 }
 async loopAmbient(c,route,delay){await wait(delay);let idx=1;while(c.scene){const p=route[idx%route.length],dir=p.x<c.x?-1:1;this.setChibiMoving(c.bodySprite,true,dir);await new Promise(res=>this.tweens.add({targets:c,x:p.x,y:p.y,duration:5600,ease:'Linear',onUpdate:()=>c.setDepth(25+c.y/100),onComplete:res}));idx++;await wait(900);}}
 ...
 setMovingSprite(actor,moving,base,dirX=1){this.setChibiMoving(actor,moving,dirX)}
 async walkContainer(c,points,follow=false){
   const actor=c.bodySprite;this.setChibiSitting(actor,false);this.setChibiMoving(actor,true,1);
   for(let i=1;i<points.length;i++){
     while(paused)await wait(70);
     const p=points[i],dir=p.x<c.x?-1:1;this.setChibiMoving(actor,true,dir);
     ...
     await new Promise(res=>this.tweens.add({targets:c,x:p.x,y:p.y,duration:Math.max(320,d*2.9),ease:'Linear',onUpdate:()=>c.setDepth(40+c.y/100),onComplete:res}));
   }
   this.setChibiMoving(actor,false,1);
 }
 standStaff(actor){this.setChibiSitting(actor,false);actor.isSitting=false}
 sitStaff(actor){this.setChibiSitting(actor,true);actor.isSitting=true;actor.scaleX=1}
 async walkStaff(actor,points){
   this.setChibiSitting(actor,false);this.setChibiMoving(actor,true,1);
   for(let i=1;i<points.length;i++){
     while(paused)await wait(70);
     const p=points[i],dir=p.x<actor.x?-1:1;this.setChibiMoving(actor,true,dir);
     const d=Phaser.Math.Distance.Between(actor.x,actor.y,p.x,p.y);
     await new Promise(res=>this.tweens.add({targets:actor,x:p.x,y:p.y,duration:Math.max(300,d*2.75),ease:'Linear',onUpdate:()=>actor.setDepth(18+actor.y/100),onComplete:res}));
   }
   this.setChibiMoving(actor,false,1);this.standStaff(actor);
 }
...
/* Campus RPG V8: high-detail vector chibi actors + premium campus + cohort orientation */
new Phaser.Game({type:Phaser.AUTO,parent:'rpg-stage',width:1600,height:900,pixelArt:false,antialias:true,roundPixels:false,backgroundColor:'#111b24',scale:{mode:Phaser.Scale.FIT,autoCenter:Phaser.Scale.CENTER_BOTH},scene:[CampusScene]});
